...
export function ArtReviewPage() {
  ...
  return <main className="min-h-screen bg-[#080d13] text-white">
    ...
    <div className="mx-auto max-w-6xl space-y-7 px-4 py-7">
      ...
      }).map((page, index) => <section key={`${page.item.id}-${page.pageIndex}`} className="art-review-sheet relative h-[794px] overflow-hidden rounded-3xl border border-white/10 bg-[#101821] p-10 shadow-2xl" style={{ backgroundImage: "linear-gradient(rgba(6,12,18,.86),rgba(6,12,18,.92)),url('/folded-courtyard-bg.jpg')", backgroundPosition: 'center', backgroundSize: 'cover' }}><div className="border-b border-white/15 pb-4"><p className="text-xs tracking-[.2em] text-[#ff8066]">{data.episode} · 第{page.analysis.sceneNo}场 · {page.item.category}</p><h2 className="mt-2 text-3xl font-semibold">{page.item.category === '场景' ? page.analysis.location || page.analysis.sceneTitle : page.item.name}</h2><p className="mt-2 text-sm text-white/60">{page.pageCount > 1 ? `Option 第${page.pageIndex + 1}/${page.pageCount}页` : '单项审阅'}</p></div><div className="mt-5 grid grid-cols-[270px_1fr] gap-6"><aside className="rounded-2xl border border-white/12 bg-black/45 p-5"><p className="text-xs tracking-[.16em] text-[#ff8066]">本场情节</p><p className="mt-3 line-clamp-5 text-base font-semibold leading-7">{page.analysis.sceneSummary || page.analysis.sceneTitle}</p><div className="mt-5 border-t border-white/10 pt-4 text-sm leading-6 text-white/60"><p className="line-clamp-5">{page.item.detail}</p>{detailByItem.get(page.item.id)?.submissionNote && <p className="mt-3 text-white/80">采用：{detailByItem.get(page.item.id)?.submissionNote}</p>}</div></aside><div className={`grid content-start place-items-center gap-4 ${page.files.length === 1 ? 'grid-cols-1' : 'grid-cols-2'}`}>{!page.files.length && <p className="rounded-xl border border-dashed border-white/20 p-10 text-sm text-white/50">本项待上传，可随时回来查看更新。</p>}{page.files.map((file, fileIndex) => { const selected = detailByItem.get(page.item.id)?.selectedFileId === file.id; return <figure key={file.id} className={`flex w-full max-w-[390px] flex-col items-center rounded-2xl border bg-black/45 p-3 ${selected ? 'border-emerald-400/70' : 'border-white/12'}`}><div className="flex h-[190px] w-full items-center justify-center"><button type="button" onClick={() => setPreviewId(file.id)} aria-label={`放大查看${file.fileName}`} className="flex h-full w-full items-center justify-center"><img src={file.url} alt={file.fileName} className="block h-auto max-h-[180px] w-auto max-w-full rounded-xl object-contain" /></button></div><figcaption className="mt-2 text-center text-xs text-white/70">{selected && <span className="mr-2 rounded-full bg-emerald-400 px-2 py-0.5 font-semibold text-black">定稿图</span>}<span>Option {page.pageIndex * 4 + fileIndex + 1}</span><span className="mt-0.5 block font-medium text-cyan-300">{uploadAuthorLabel(file.uploadedBy)}</span></figcaption></figure>})}</div></div><Footer episode={data.episode} page={index + 2} /></section>)}
    </div>
  </main>;
}
