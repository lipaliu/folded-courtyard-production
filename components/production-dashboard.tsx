...
const roles = ['编剧', '主美', 'AIGC抽卡师', '制片人（叶总）', '红人（Yoyo）', '联合制片人／导演：Lipa'];
...
export function ProductionDashboard() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <Tabs defaultValue="today" className="mx-auto min-h-screen w-full max-w-6xl pb-24 md:pb-9">
        ...
        <div className="px-4 py-5 md:px-8 md:py-8">
          <div className="mx-auto max-w-6xl">
            <TabsContent value="today" className="mt-0"><TodayView selectedDate={selectedDate} setSelectedDate={setSelectedDate} items={todayItems} progress={dayProgress} completed={completed} planned={planned} risks={risks} isAdmin={Boolean(me?.isAdmin)} onEdit={setSelectedItem} onAdd={setCreatingRole} /></TabsContent>
      ...
      <NewTaskEditor role={creatingRole} workDate={selectedDate} open={Boolean(creatingRole)} onClose={() => setCreatingRole(null)} onSave={async (draft) => { await createItem(draft); setCreatingRole(null); }} />
...
function TodayView({ selectedDate, setSelectedDate, items, progress, completed, planned, risks, isAdmin, onEdit, onAdd }: { selectedDate: string; setSelectedDate: (value: string) => void; items: ProductionItem[]; progress: number; completed: number; planned: number; risks: number; isAdmin: boolean; onEdit: (item: ProductionItem) => void; onAdd: (role: string) => void }) {
  ...
  return <>
    <section className="grid gap-4 md:grid-cols-[1.45fr_.75fr]">
      <div className="control-card overflow-hidden p-5 md:p-7">
        <div className="flex items-start justify-between gap-4">
          ...
          <div className="relative grid h-20 w-20 shrink-0 place-items-center rounded-full" style={{ background: `conic-gradient(#ff6240 0 ${progress}%, rgba(255,255,255,.08) ${progress}% 100%)` }}>
            <div className="grid h-[66px] w-[66px] place-items-center rounded-full bg-card"><div className="text-center"><b className="text-xl">{progress}%</b><span className="block text-[10px] text-muted-foreground">今日进度</span></div></div>
          </div>
        </div>
        <div className="mt-6 grid grid-cols-3 gap-2 border-t border-white/8 pt-4">
          ...
          <Metric label="风险" value={String(risks)} suffix="项" />
        </div>
      </div>
      <div className="control-card p-5 md:p-6">
        ...
        <input type="date" min="2026-09-09" max="2026-10-09" value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} className="mt-4 h-11 w-full rounded-lg border border-white/10 bg-white/5 px-3 text-base outline-none focus:border-[#ff6240]" />
        <p className="mt-5 text-3xl font-semibold tracking-[-.04em]">10月9日</p><p className="mt-1 text-sm text-muted-foreground">全片硬交付 · 剩余30天</p>
      </div>
    </section>
    ...
    <section className="mt-7 rounded-2xl border border-[#ff6240]/20 bg-[#ff6240]/8 p-4 md:p-5"><div className="flex gap-3"><div className="mt-0.5 grid h-8 w-8 shrink-0 place-items-center rounded-full bg-[#ff6240] text-black"><ChevronRight className="h-4 w-4" /></div><div><p className="font-medium">生产规则</p><p className="mt-1 text-sm leading-6 text-muted-foreground">上一批正式镜头生成时，下一批剧本、场景、服装、配角和白模必须同步完成。</p></div></div></section>
  </>;
}
...
function PlanView({ items, batches, isAdmin, onEdit }: { items: ProductionItem[]; batches: PlanBatch[]; isAdmin: boolean; onEdit: (batch: PlanBatch) => void }) {
  return <section>
    <div className="mb-5 flex items-end justify-between"><div><p className="eyebrow">MASTER PLAN · 9.09—10.09</p><h2 className="mt-1 text-2xl font-semibold">一个月滚动大计划</h2></div><span className="hidden text-sm text-muted-foreground sm:block">筹备与生产永远错开</span></div>
...
function TaskEditor({ item, open, saving, onClose, onDelete, onSave }: { item: ProductionItem | null; open: boolean; saving: boolean; onClose: () => void; onDelete: () => Promise<void>; onSave: (changes: Partial<ProductionItem>) => Promise<void> }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-end bg-black/70 backdrop-blur-sm sm:place-items-center sm:p-4" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose(); }}><section role="dialog" aria-modal="true" aria-labelledby="task-editor-title" className="max-h-[92vh] w-full max-w-xl overflow-y-auto rounded-t-[24px] border border-white/10 bg-[#1b1d22] p-5 shadow-2xl sm:rounded-[20px]"><div className="flex items-start justify-between gap-4"><div><h2 id="task-editor-title" className="text-lg font-medium">编辑任务</h2><p className="mt-1 text-sm text-muted-foreground">日期、时间、负责人、数量和备注都可以调整</p></div><button onClick={onClose} aria-label="关闭" className="grid h-8 w-8 place-items-center rounded-full bg-white/5"><X className="h-4 w-4" /></button></div><div className="mt-5 space-y-4"><Field label="任务名称"><input value={draft.title} onChange={(event) => set('title', event.target.value)} className="edit-input" /></Field><div className="grid grid-cols-2 gap-3"><Field label="工作日期"><input type="date" value={draft.workDate} onChange={(event) => set('workDate', event.target.value)} className="edit-input" /></Field><Field label="截止时间"><input type="time" value={draft.dueTime} onChange={(event) => set('dueTime', event.target.value)} className="edit-input" /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="负责人"><select value={draft.owner} onChange={(event) => set('owner', event.target.value)} className="edit-input">{roles.map((role) => <option key={role} value={role}>{role === 'AIGC抽卡师' ? '抽卡师' : role}</option>)}</select></Field><Field label="状态"><select value={draft.status} onChange={(event) => set('status', event.target.value as Status)} className="edit-input">{STATUSES.map((value) => <option key={value} value={value}>{value}</option>)}</select></Field></div><div className="grid grid-cols-2 gap-3"><Field label="集数"><input value={draft.episode} onChange={(event) => set('episode', event.target.value)} className="edit-input" /></Field><Field label="工作类型"><input value={draft.category} onChange={(event) => set('category', event.target.value)} className="edit-input" /></Field></div><div className="grid grid-cols-2 gap-3"><Field label="计划量"><input type="number" min="0" value={draft.plannedQty} onChange={(event) => set('plannedQty', Number(event.target.value))} className="edit-input" /></Field><Field label="实际完成"><input type="number" min="0" value={draft.completedQty} onChange={(event) => set('completedQty', Number(event.target.value))} className="edit-input" /></Field></div><Field label="同步统筹 / 备注 / 打回原因"><Textarea value={draft.note} onChange={(event) => set('note', event.target.value)} rows={3} /></Field></div><div className="mt-5 grid grid-cols-[auto_1fr_1fr] gap-2"><Button variant="destructive" className="h-11" onClick={() => void onDelete()}>删除</Button><Button variant="outline" className="h-11" onClick={onClose}>取消</Button><Button className="h-11" disabled={saving} onClick={() => void onSave(draft)}>{saving ? <Loader2 className="animate-spin" /> : <Check />}保存并同步</Button></div></section></div>;
}
...
function RoleTask({ item, editable, onEdit }: { item: ProductionItem; editable: boolean; onEdit: () => void }) {
  return <button disabled={!editable} onClick={onEdit} className="flex w-full items-center gap-3 rounded-xl border border-white/7 bg-white/[.025] p-3 text-left enabled:hover:bg-white/5 disabled:cursor-default"><span className={`h-2 w-2 shrink-0 rounded-full ${item.status === '已通过' ? 'bg-emerald-400' : item.status === '待审核' ? 'bg-violet-400' : item.status === '打回' ? 'bg-red-400' : 'bg-[#ff6240]'}`} /><div className="min-w-0 flex-1"><p className="truncate text-sm">{item.title}</p><p className="mt-1 text-[11px] text-muted-foreground">{item.episode} · {item.dueTime} · {item.completedQty}/{item.plannedQty}</p></div><span className={`rounded-full border px-2 py-1 text-[10px] ${statusStyle[item.status]}`}>{item.status}</span></button>;
}
...
function NewTaskEditor({ role, workDate, open, onClose, onSave }: { role: string | null; workDate: string; open: boolean; onClose: () => void; onSave: (draft: Partial<ProductionItem>) => Promise<void> }) {
  ...
  const [note, setNote] = useState('');
  useEffect(() => { if (open) { setTitle(''); setDate(workDate); setTime('18:00'); setNote(''); setCategory(role === '编剧' ? '剧本' : role === '主美' ? '场景图' : role === 'AIGC抽卡师' ? '正式镜头' : role?.includes('Yoyo') ? '审核' : '统筹'); } }, [open, role, workDate]);
  if (!open || !role) return null;
  return <div className="fixed inset-0 z-50 grid place-items-end bg-black/70 backdrop-blur-sm sm:place-items-center sm:p-4"><section role="dialog" aria-modal="true" className="w-full max-w-lg rounded-t-[24px] border border-white/10 bg-[#1b1d22] p-5 sm:rounded-[20px]"><div className="flex items-center justify-between"><div><h2 className="text-lg font-medium">给{role === 'AIGC抽卡师' ? '抽卡师' : role}安排工作</h2><p className="mt-1 text-sm text-muted-foreground">新增后全组同步可见</p></div><button onClick={onClose} className="grid h-8 w-8 place-items-center rounded-full bg-white/5"><X className="h-4 w-4" /></button></div><div className="mt-5 space-y-4"><Field label="任务名称"><input autoFocus value={title} onChange={(event) => setTitle(event.target.value)} className="edit-input" placeholder="今天必须完成什么" /></Field><div className="grid grid-cols-2 gap-3"><Field label="日期"><input type="date" value={date} onChange={(event) => setDate(event.target.value)} className="edit-input" /></Field><Field label="截止时间"><input type="time" value={time} onChange={(event) => setTime(event.target.value)} className="edit-input" /></Field></div><div className="grid grid-cols-3 gap-3"><Field label="集数"><input value={episode} onChange={(event) => setEpisode(event.target.value)} className="edit-input" /></Field><Field label="类型"><input value={category} onChange={(event) => setCategory(event.target.value)} className="edit-input" /></Field><Field label="计划量"><input type="number" min="0" value={qty} onChange={(event) => setQty(Number(event.target.value))} className="edit-input" /></Field></div><Field label="同步统筹备注"><Textarea value={note} onChange={(event) => setNote(event.target.value)} rows={3} placeholder="交接条件、风险、前置依赖……" /></Field></div><div className="mt-5 grid grid-cols-2 gap-2"><Button variant="outline" className="h-11" onClick={onClose}>取消</Button><Button className="h-11" disabled={!title.trim()} onClick={() => void onSave({ title, workDate: date, dueTime: time, episode, category, plannedQty: qty, owner: role, reviewer: 'Yoyo', note })}><Check />添加并同步</Button></div></section></div>;
}
