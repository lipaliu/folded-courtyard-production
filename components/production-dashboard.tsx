  ...
  return <>
    <section className="grid gap-4 md:grid-cols-[1.45fr_.75fr]">
      <div className="control-card overflow-hidden p-5 md:p-7">
        ...
        <div className="mt-6 grid grid-cols-3 gap-2 border-t border-white/8 pt-4">
          ...
          <Metric label="前置卡住" value={String(blocked + risks)} suffix="项" />
        </div>
      </div>
    ...
    {items.length > 0 && <section className="mt-7">
      ...
      <div className="flex gap-2 overflow-x-auto pb-2">{items.slice().sort((a, b) => a.sortOrder - b.sortOrder).map((item, index) => {
        ...
        return <div key={item.id} className="flex shrink-0 items-center gap-2"><button disabled={!isAdmin} onClick={() => onEdit(item)} className={`w-52 rounded-xl border p-3 text-left disabled:cursor-default ${state.blocked ? 'border-amber-400/25 bg-amber-400/8' : item.status === '已通过' ? 'border-emerald-400/20 bg-emerald-400/8' : 'border-white/8 bg-card'}`}><p className="text-[10px] text-muted-foreground">{item.owner} · {item.dueTime}</p><p className="mt-1 truncate text-sm font-medium">{item.title}</p><p className={`mt-2 text-[10px] ${state.blocked ? 'text-amber-300' : 'text-muted-foreground'}`}>{state.blocked ? `等待${state.upstream?.owner} ${state.upstream?.dueTime}提报` : item.handoffTo ? `交给${item.handoffTo} · ${item.handoffDeadline}` : '独立任务'}</p></button>{index < items.length - 1 && <ChevronRight className="h-4 w-4 shrink-0 text-zinc-700" />}</div>;
      })}</div>
    </section>}
...
function RoleTask({ item, allItems, editable, onEdit }: { item: ProductionItem; allItems: ProductionItem[]; editable: boolean; onEdit: () => void }) {
  const state = dependencyState(item, allItems);
  return <button disabled={!editable} onClick={onEdit} className={`flex w-full items-start gap-3 rounded-xl border p-3 text-left enabled:hover:bg-white/5 disabled:cursor-default ${state.blocked ? 'border-amber-400/20 bg-amber-400/[.045]' : 'border-white/7 bg-white/[.025]'}`}><span className={`mt-1 h-2 w-2 shrink-0 rounded-full ${state.blocked ? 'bg-amber-400' : item.status === '已通过' ? 'bg-emerald-400' : item.status === '待审核' ? 'bg-violet-400' : item.status === '打回' ? 'bg-red-400' : 'bg-[#ff6240]'}`} /><div className="min-w-0 flex-1"><p className="text-sm">{item.title}</p><p className="mt-1 text-[11px] text-muted-foreground">{item.episode} · 本人 {item.dueTime} · {item.completedQty}/{item.plannedQty}</p><p className={`mt-1.5 text-[10px] ${state.blocked ? 'text-amber-300' : 'text-zinc-500'}`}>{state.blocked ? `前置未交：${state.upstream?.owner}须在${state.upstream?.dueTime}前提报` : item.handoffTo ? `交给：${item.handoffTo} · ${item.handoffDeadline}` : '无后续交接'}</p></div><span className={`shrink-0 rounded-full border px-2 py-1 text-[10px] ${state.blocked ? 'border-amber-400/25 bg-amber-400/10 text-amber-300' : statusStyle[item.status]}`}>{state.blocked ? '等待前置' : item.status}</span></button>;
}
