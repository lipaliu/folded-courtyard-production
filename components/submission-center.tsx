...
function SubmissionItemCard({ item, detail, files, reuseInfo, defaultDueAt, canEdit, canDelete, selectedForDelete, onToggleDelete, isAdmin, currentName, onUpload, onSave, onDeleteFile, onDeleteItem }: { item: ScriptAssetItem; detail?: SubmissionDetail; files: SubmissionFile[]; reuseInfo?: ReuseInfo; defaultDueAt: string; canEdit: boolean; canDelete: boolean; selectedForDelete: boolean; onToggleDelete: () => void; isAdmin: boolean; currentName: string; onUpload: (itemId: string, files: File[], onProgress?: (done: number) => void) => Promise<number>; onSave: (itemId: string, changes: Partial<SubmissionDetail>) => Promise<void>; onDeleteFile: (file: SubmissionFile) => Promise<void>; onDeleteItem: (item: ScriptAssetItem) => Promise<void> }) {
  ...
  async function upload(selectedFiles: File[]) {
    ...
    setUploading(true); setUploadProgress({ done: 0, total: selectedFiles.length }); setError('');
    try {
      const count = await onUpload(item.id, selectedFiles, (done) => setUploadProgress({ done, total: selectedFiles.length }));
      setUploadProgress({ done: count, total: selectedFiles.length });
      setDraft((current) => ({ ...current, status: '已上传' }));
    } catch (nextError) {
  ...
  async function selectFinal(file: SubmissionFile) {
    ...
    try {
      ...
      setDraft((current) => ({ ...current, status: clearing ? '已上传' : '已锁定' }));
    } catch (nextError) {
