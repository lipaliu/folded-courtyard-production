...
type SubmissionDetail = { itemId: string; assignedTo: string; dueAt: string; handoffTo: string; doneDefinition: string; status: string; submissionNote: string; reviewNote: string; selectedFileId: string; submittedAt: string; reviewedAt: string; updatedAt: string };
    ...
    for (const file of selectedFiles) {
      try {
        ...
        setDetails((current) => current.some((detail) => detail.itemId === itemId)
          ...
          : [...current, { itemId, assignedTo: '', dueAt: '', handoffTo: 'Lipa', doneDefinition: '', status: '已上传', submissionNote: '', reviewNote: '', selectedFileId: '', submittedAt: '', reviewedAt: '', updatedAt: '', ...data.detail }]);
        uploadedCount += 1;
...
function SubmissionPdfSource({ id, episode, workDate, version, analyses, items, details, filesByItem }: { id: string; episode: string; workDate: string; version?: ScriptVersion; analyses: ScriptAnalysis[]; items: ScriptAssetItem[]; details: SubmissionDetail[]; filesByItem: Map<string, SubmissionFile[]> }) {
  ...
  for (const analysis of analyses) {
    const sceneEntries = episodeItems
      ...
      .flatMap((item) => (filesByItem.get(item.id) || []).map((file) => ({ item, file, selected: selectedByItem.get(item.id) === file.id })));
    ...
    for (const item of reviewItems) {
      ...
      fileGroups.forEach((pageFiles, index) => contentPages.push({ kind: 'item', analysis, item, files: pageFiles, selectedFileId: selectedByItem.get(item.id) || '', pageIndex: index + 1, pageCount: fileGroups.length, itemReferenceCount: itemFiles.length }));
    }
  }
