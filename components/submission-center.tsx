...
export function SubmissionCenter({ me, selectedDate, productionItems, onAssigned, mode = 'art' }: { me: CurrentUser; selectedDate: string; productionItems: ProductionItem[]; onAssigned: (date: string) => Promise<void>; mode?: 'script' | 'art' }) {
  ...
  return <section>
    <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">ART UPLOAD & REVIEW</p><h2 className="mt-1 text-2xl font-semibold">主美上传与美术审图</h2><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">主美只负责按场次上传人物、服装、道具和场景参考；Lipa确认齐套后，独立生成本集美术提报H5或PDF。</p></div><button onClick={() => void loadAll()} className="grid h-10 w-10 place-items-center rounded-full border border-white/10 bg-white/5 text-muted-foreground" aria-label="刷新美术档案"><RefreshCw className="h-4 w-4" /></button></div>
    ...
    {me.isAdmin && <section className="control-card mt-4 p-4 md:p-6"><div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between"><div><p className="text-sm font-medium">1 · Lipa安排当天场次</p><p className="mt-1 text-xs text-muted-foreground">定稿场次排入当天后，主美按下面的资产卡上传。</p></div><div className="flex flex-wrap items-center gap-2">{analyses.map((analysis) => { const selected = selectedIds.includes(analysis.id); const assigned = assignedIds.has(analysis.id); return <button key={analysis.id} disabled={saving} onClick={() => setSelectedIds((current) => selected ? current.filter((id) => id !== analysis.id) : [...current, analysis.id])} className={`rounded-lg border px-3 py-2 text-xs ${selected ? 'border-[#ff6240] bg-[#ff6240]/15 text-white' : assigned ? 'border-emerald-400/25 bg-emerald-400/10 text-emerald-300' : 'border-white/10 bg-white/5 text-muted-foreground'}`}>{assigned ? '已排 ' : selected ? '✓ ' : '□ '}{analysis.episode}·{analysis.sceneNo}场</button>; })}<Button size="sm" disabled={!selectedIds.length || saving} onClick={() => void assignExisting()}><Clock3 />排入{formatDate(workDate)}</Button></div></div></section>}

    {notice && <p className="mt-4 rounded-xl border border-emerald-400/20 bg-emerald-400/[.06] px-4 py-3 text-sm text-emerald-300">{notice}</p>}
    {error && <p className="mt-4 rounded-xl border border-red-400/20 bg-red-400/[.06] px-4 py-3 text-sm text-red-300">{error}</p>}

    <div className="mt-7 flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">DAILY ART PACKAGE</p><h2 className="mt-1 text-xl font-semibold">{formatDate(workDate)} · 人服道景上传清单</h2><p className="mt-1 text-xs text-muted-foreground">{dailyAnalyses.length}场 · {readyCount}/{dailyItems.length}项齐套 · 主美上传，Lipa最终审图和提报</p></div>{me.isAdmin && <div className="flex flex-wrap gap-2">{dailyEpisodes.map((episode) => <div key={episode} className="flex gap-2"><a href={`/art-review?episode=${encodeURIComponent(episode)}`} target="_blank" rel="noreferrer" className={`inline-flex h-10 items-center gap-2 rounded-lg border px-3 text-sm ${readyCount === dailyItems.length && dailyItems.length ? 'border-cyan-300/25 bg-cyan-300/8 text-cyan-200' : 'pointer-events-none border-white/8 text-zinc-600'}`}><ExternalLink className="h-4 w-4" />生成美术H5</a><Button variant="outline" disabled={pdfEpisode === episode || readyCount !== dailyItems.length || !dailyItems.length} onClick={() => void downloadPdf(episode)}>{pdfEpisode === episode ? <Loader2 className="animate-spin" /> : <Download />}{`下载${episode}美术PDF`}</Button></div>)}</div>}</div>
    ...
    <div id="art-upload-list" className="mt-4 scroll-mt-6 space-y-4">{dailyAnalyses.length ? dailyAnalyses.map((analysis) => { const sceneItems = items.filter((item) => item.analysisId === analysis.id).sort((a, b) => a.sortOrder - b.sortOrder); return <article key={analysis.id} className="control-card p-4 md:p-5"><div className="border-b border-white/8 pb-4"><p className="text-xs text-[#ff8066]">{analysis.episode} · 第{analysis.sceneNo}场</p><h3 className="mt-1 text-lg font-medium">{analysis.sceneTitle}</h3><p className="mt-1 text-sm leading-6 text-muted-foreground">{analysis.location} · {analysis.sceneSummary}</p></div><div className="mt-4 grid gap-3 xl:grid-cols-2">{sceneItems.map((item) => <SubmissionItemCard key={item.id} item={item} detail={detailMap.get(item.id)} files={resolvedFilesByItem.get(item.id) || []} reuseInfo={reuseByItem.get(item.id)} defaultDueAt={`${workDate}T18:00`} canEdit={canEditArt} canDelete={canDeleteProps && item.category === '道具'} selectedForDelete={selectedPropIds.includes(item.id)} onToggleDelete={() => setSelectedPropIds((current) => current.includes(item.id) ? current.filter((id) => id !== item.id) : [...current, item.id])} isAdmin={me.isAdmin} currentName={me.name} onUpload={uploadImage} onSave={saveDetail} onDeleteFile={deleteFile} onDeleteItem={deleteProp} />)}</div></article>; }) : <Empty text={`${formatDate(workDate)}还没有具体场次。Lipa上传单集剧本，或在上方选择场次排入当天。`} />}</div>
