...
export async function POST(request: Request) {
  ...
  try {
    await env.DB.batch([
      env.DB.prepare(`INSERT INTO member_accounts
        ...
        VALUES (?, ?, ?, ?, ?, ?, ?, 0, 1, 0, '', ?, ?)`)
        .bind(id, username, credentials.hash, credentials.salt, credentials.iterations, name, role, now, now),
  ...
  return Response.json({ user: { id, username, name, role, isAdmin: false } }, { headers: { 'Set-Cookie': cookie } });
}
