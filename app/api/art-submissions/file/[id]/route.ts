...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const file = await env.DB.prepare(`SELECT object_key AS objectKey, file_name AS fileName, content_type AS contentType, file_data AS fileData
    FROM art_submission_files WHERE id = ?`).bind(id).first<{ objectKey: string; fileName: string; contentType: string; fileData: ArrayBuffer | null }>();
  ...
  if (file.objectKey.startsWith('d1:')) {
    ...
    return new Response(file.fileData, { headers: imageHeaders(file.contentType, file.fileName) });
  }
