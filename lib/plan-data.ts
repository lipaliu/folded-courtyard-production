...
const special: ProductionItem[] = [
  ...
  item('0910-send-yoyo', '2026-09-10', '第1集', '审核交接', '检查首批视觉并发微信给Yoyo', '联合制片人／导演：Lipa', '未开始', 1, 0, '12:15', '收到主美提报后立即发送；发出后等待微信回复', 3, '0910-art', '红人（Yoyo）', '发出即等待回复'),
  item('0910-yoyo', '2026-09-10', '第1集', '红人审核', '在微信反馈第一集首批视觉设定', '红人（Yoyo）', '未开始', 5, 0, '微信待回复', 'Yoyo不登录系统、不设硬deadline；未回复不阻断剧本、场景图和白模', 4, '0910-send-yoyo', '联合制片人／导演：Lipa', '收到回复后更新'),
  ...
  item('0911-yoyo-white', '2026-09-11', '第1集', '美术图审核', '在微信反馈第一集主美视觉图', '红人（Yoyo）', '未开始', 5, 0, '微信待回复', 'Yoyo只看主美做的图，不审核剧本和白模；Lipa收到意见后逐项更新', 9, '0910-send-yoyo', '联合制片人／导演：Lipa', '收到回复后更新'),
  item('0911-producer-white', '2026-09-11', '第1集', '美术图审核', '审核第一集主美视觉图', '制片人（叶总）', '未开始', 5, 0, '18:00', '叶总只看主美做的图，不审核剧本和白模', 10, '0910-send-yoyo', '联合制片人／导演：Lipa', '18:15'),
  item('0911-review', '2026-09-11', '第1集', '审核汇总', '汇总Yoyo与叶总对主美图的意见', '联合制片人／导演：Lipa', '未开始', 1, 0, '20:00', '按已收到的意见逐项更新；最终视觉锁定后释放正式镜头', 11, '0910-send-yoyo', 'AIGC抽卡师', '2026-09-12 09:00'),
];
...
const rolling = batches.flatMap((batch, batchIndex) =>
  datesBetween(batch.start, batch.end).flatMap((date, dayIndex) => {
    ...
    rows.push(
      ...
      item(`${date}-review`, date, `${batch.production} / ${batch.prep}`, '红人审核', '在微信反馈当日视觉与成片意见', '红人（Yoyo）', '未开始', 1, 0, '微信待回复', 'Yoyo不登录系统、不设硬deadline；未回复不阻断剧本、场景图和白模，只影响最终锁定', base + 5, batch.prep === '全片' ? '' : `${date}-prep`, '联合制片人／导演：Lipa', '收到回复后更新'),
...
export const initialItems: ProductionItem[] = [
  ...special,
  ...rolling,
  item('1021-delivery', '2026-10-21', '全片', '成片', '全片总审、修正与最终交付', '联合制片人／导演：Lipa', '未开始', 10, 0, '20:00', '按六休一与国庆假期重排，给第10集保留生成、初剪、修改成片三天', 999, '', '制片人（叶总）', '20:00'),
];
